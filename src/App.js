import './App.css';
import React, {useState} from "react"
import {click} from '@testing-library/user-event/dist/click';

function App() {

const [number1, setNumber1] = useState("");
const [number2, setNumber2] = useState("");
const [OperacionActual, setOperacionActual] = useState("");
const [result, setResult] = useState(0);

function limpiar(){
  setNumber1("");
  setNumber2("");
  setOperacionActual("");
  setResult("");
}

function DEL(){
    if (OperacionActual === "") {
      setNumber1(number1.toString().slice(0, -1));
    } else {
      setNumber2(number2.toString().slice(0, -1));
    };
}

function clickNumber (val) {
  if (OperacionActual === "") {
    setNumber1(number1 + val)
  } else {
    setNumber2(number2 + val)
  }
}
function clickOperation (val){
  setOperacionActual(val);
}
function getResult (){
  switch(OperacionActual){
    case "+":
      setResult(Number(number1) + Number(number2));
      break;
    case "-":
      setResult(Number(number1) - Number(number2));
      break;
    case "*":
        setResult(Number(number1) * Number(number2));
      break;
    case "/":
        setResult(Number(number1) / Number(number2));
      break;
  }
}
  return (
    <div className="App">
      <h1 classname="titulo">Calculadora Diego Rangel</h1>
      <div className="Calculadora-grid">
        <div className="output">
          <div className="OperacionPrevia">{OperacionActual ? number1 + OperacionActual : ""}</div>
          <div className="OperacionActual">{result ? result :(!OperacionActual ? number1 : number2)}</div>
        </div>
        <button onClick={limpiar} classname="span-two">AC</button>
        <button onClick={DEL}>DEL</button>
        <button onClick={() =>{clickOperation("/")}}>/</button>
        <button onClick={()=> {clickNumber(7)}}>7</button>
        <button onClick={()=> {clickNumber(8)}}>8</button>
        <button onClick={()=> {clickNumber(9)}}>9</button>
        <button onClick={() =>{clickOperation("*")}}>*</button>
        <button onClick={()=> {clickNumber(4)}}>4</button>
        <button onClick={()=> {clickNumber(5)}}>5</button>
        <button onClick={()=> {clickNumber(6)}}>6</button>
        <button onClick={() =>{clickOperation("+")}}>+</button>
        <button onClick={()=> {clickNumber(1)}}>1</button>
        <button onClick={()=> {clickNumber(2)}}>2</button>
        <button onClick={()=> {clickNumber(3)}}>3</button>
        <button onClick={() =>{clickOperation("-")}}>-</button>
        <button onClick={() =>{clickNumber(".")}}>.</button>
        <button onClick={() =>{clickNumber(0)}}>0</button>
        <button onClick={getResult}Classname="span-two">=</button>        
      </div>
    </div>
  );
}

export default App;
